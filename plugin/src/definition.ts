...
export function pulseDefinitionFromForm(input: PulseDefinitionInput): Record<string, unknown> {
  ...
  return { id, title, active: input.active ?? true, schedule: { type: "weekly", daysOfWeek: [input.day], time: input.time, timezone: input.timezone }, notificationPolicy: { channels: ["ntfy"], repeatEveryMinutes: deliveryRetryMinutes, snoozeEveryMinutes } };
}
